...
The 15-test suite covers the valid canonical path, the four deterministic check dimensions, Unicode-subscript expression input, first-invalid-step stopping behavior, missing steps, graph consistency, evidence validation, persistence fallbacks, JSON export, and key UI outcomes. The graph tests verify both dependency existence and order and ensure the step map has no orphan definitions.
