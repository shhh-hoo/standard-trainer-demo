...
The V2 deterministic core adds thirty-three runtime tests around fail-closed validation, AST evaluation, recognition gating, exact 16-fixture diagnosis, embedded-formula adversaries, display-symbol metamorphism, independent evidence, revision-order invariance, support/check separation, trace consistency, assistance causality, the V1 adapter, and four typed mock scenarios. Exact gold match plus adversarial coverage demonstrates execution of the authored bounded contract; it does not demonstrate curriculum coverage.
...
The V2 typed-working adapter is explicitly a mock scenario selector, not NLP. The diagnosis engine explicitly supports only the current gold problem definition rather than presenting its Kp-specific implementation as topology-generic. The repository still contains no OCR, real parser, server endpoint, or model call, and the public live learner experience remains V0.1.
