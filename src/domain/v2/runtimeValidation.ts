...
export function validateDiagnosticProblemDefinitionV2(
  ...
  if (target) {
    ...
    const significantFigures = requireFinite(target.significantFigures, "$.target.significantFigures", issues);
    if (significantFigures !== null && (!Number.isInteger(significantFigures) || significantFigures < 1)) {
      issue(issues, "$.target.significantFigures", "INVALID_SIGNIFICANT_FIGURES", "Must be a positive integer.");
    }
  }
  ...
  if (graph) {
    ...
    Object.entries(nodes).forEach(([key, value], index) => {
      ...
      requireArray(node.dependencies, `$.reasoningGraph.nodes.${key}.dependencies`, issues).forEach((dependency, dependencyIndex) => {
        ...
        } else if (order.indexOf(dependency) >= index) {
      ...
      requireArray(node.solutionEvidenceKinds, `$.reasoningGraph.nodes.${key}.solutionEvidenceKinds`, issues);
...
export function validateNormalizedAttempt(
  ...
  const artifactIds = artifactValues.map((artifact, index) => {
    ...
    if (record.modality === "HANDWRITING_IMAGE" || record.modality === "DIGITAL_INK") {
      const pages = requireFinite(record.pageCount, `$.artifacts[${index}].pageCount`, issues);
      if (pages !== null && (!Number.isInteger(pages) || pages < 1)) issue(issues, `$.artifacts[${index}].pageCount`, "INVALID_PAGE_COUNT", "Page count must be positive.");
    }
    return id;
  });
  ...
  const revisionIds = revisionValues.map((revision, index) => {
    ...
    requireFinite(record.sequence, `$.revisions[${index}].sequence`, issues);
    requireString(record.submittedAt, `$.revisions[${index}].submittedAt`, issues);
  ...
  const stepIds = stepValues.map((step, index) => {
    ...
    validateRecognition(record.recognition, `$.steps[${index}].recognition`, issues, problem.recognitionPolicy.autoAcceptThreshold);
    ...
    if (record.calculation !== undefined) {
      ...
      if (calculation) {
        ...
        if (calculation.declaredResult !== undefined) {
          const result = requireRecord(calculation.declaredResult, `$.steps[${index}].calculation.declaredResult`, issues);
          if (result) requireFinite(result.value, `$.steps[${index}].calculation.declaredResult.value`, issues);
        }
      }
    }
    return id;
  ...
  if (attempt.target !== null) {
    ...
    if (target) requireArray(target.evidenceStepIds, "$.target.evidenceStepIds", issues).forEach((id, index) => {
  ...
  const assistanceIds = assistanceValues.map((event, index) => {
    ...
    requireFinite(record.sequence, `$.assistanceEvents[${index}].sequence`, issues);
    ...
    requireString(record.timestamp, `$.assistanceEvents[${index}].timestamp`, issues);
    return id;
  });
  ...
  const recognitionIssueIds = recognitionIssues.map((recognitionIssue, index) => {
    ...
    if (record.scope === "STEP") {
      ...
      validateRecognition(record.recognition, `$.recognitionIssues[${index}].recognition`, issues, problem.recognitionPolicy.autoAcceptThreshold);
  ...
  if (attempt.finalAnswer !== null) {
    ...
    if (finalAnswer) {
      requireFinite(finalAnswer.value, "$.finalAnswer.value", issues);
...
export function validateDiagnosticEvidenceTraceV2(
  ...
  requireString(trace.submittedAt, "$.submittedAt", issues);
  ...
  requireArray(trace.alignmentEvidence, "$.alignmentEvidence", issues);
  requireArray(trace.deterministicChecks, "$.deterministicChecks", issues);
