...
export default function App() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const problemContext = { prompt: component.presentation.prompt, reactionEquation: component.presentation.reaction ?? component.presentation.title, givenValues: component.authoredFacts.filter((fact): fact is typeof fact & { value: number } => typeof fact.value === "number").map((fact) => ({ label: fact.label, value: fact.value, unit: fact.unit ?? "1" })), targetQuantity: component.presentation.title, answerRequirement: `${component.target.significantFigures} significant figures` };
    const problemContextEvidence = { promptQuote: problemContext.prompt, reactionEquationQuote: problemContext.reactionEquation, givenValueQuotes: problemContext.givenValues.map((given) => `${given.label}: ${given.value} ${given.unit}`), targetQuantityQuote: problemContext.targetQuantity, answerRequirementQuote: problemContext.answerRequirement };
    let nextTrace: LearnerEvidenceTrace;
    try {
      const result = await runLearnerDiagnosis({ runPurpose: "PRODUCT", componentId: component.id, componentVersion: component.version, problemContext, problemContextEvidence, attempt }, { registry: activeRegistry });
